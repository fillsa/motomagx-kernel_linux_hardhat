/* 
   ...
   2007-Jan-24  Motorola         Added modkconfig header file
   2007-Mar-29  Motorola         Incase of incoming disconnect request, not set the struct sock->sk_err value to ECONNRESET
*/
...
static void l2cap_sock_kill(struct sock *sk)
{
	...
	BT_DBG("sk %p sk->sk_refcnt %d",sk,sk->sk_refcnt); 
	sock_put(sk);
}
...
static inline int l2cap_disconnect_req(struct l2cap_conn *conn, struct l2cap_cmd_hdr *cmd, u8 *data)
{
	...
	/* instead of using l2cap_chan_del(sk, ECONNRESET) */
	l2cap_chan_del(sk,0);
	bh_unlock_sock(sk);
