 ...
 *
 * Date     Author      Comment
 * 08/01/2006   Motorola    Creat initial version
 * 02/08/2007   Motorola    Edit ltt_lite_ev_schedchange()
 * 03/13/2007   Motorola    Add debug printk macro, buffer control struct and related function
 * 03/26/2007   Motorola    Add ltt_lite_get_ms_time() and ltt_lite_log_string()
 * 04/05/2007   Motorola    Add support for CONFIG_MOT_FEAT_PRIVATE_RAM
 * 04/20/2007   Motorola    Modify get_privateram_address()
 * 05/21/2007   Motorola    Modify if_log_syscall(), add ltt_lite_log_syscall() and ltt_lite_syscall_param()
 * 07/02/2007   Motorola    Add signal log structures
 * 07/23/2007   Motorola    Add ltt_lite_printf()
 * 11/29/2007   Motorola    Modify ltt_lite_parser_mem()
 * 12/05/2007   Motorola    Modify write_early_buf()
 * 12/17/2007   Motorola    Change the log_file[]
 * 12/18/2007   Motorola    Modify the initial pagenum in ltt_lite_cmdline_setup()
 * 12/18/2007   Motorola    Add process_name_table_init() when early_enabled_mode is equal to true
 * 12/18/2007   Motorola    Change ltt_lite_early_page_num to 1024
 * 12/18/2007   Motorola    Modify ltt_lite_cmdline_setup()
 * 12/18/2007   Motorola    Change LTT_LITE_PAGE_ORDER to 7
 * 12/19/2007   Motorola    Delete the support CONFIG_MOT_FEAT_PRIVATE_RAM in ltt_lite_init()
 * 12/21/2007   Motorola    Initial version for ljos_7.2
 *
 * 01/2008  Motorola	Add support for LTT-lite syscall
 */
...
#define TASK_COMM_LEN (sizeof((struct task_struct *)0)->comm)
 ...
 */
static char log_file[LTT_LITE_CMD_LEN] = "/ezxlocal/ltt_lite_profilelog";
...
static void wq_log_top_half(void* ignore);
/* the function that logs the bottom buf */
static void wq_log_bottom_half(void* ignore);
/* the function that executes the initialization of workqueue */
static void wq_init(void* ignore);
/* the function that executes close file and disable profile */
static void wq_close_file(void* ignore);
/* declare the workqueues of the above four functions */
static DECLARE_WORK(wq_log_top_half_work, wq_log_top_half, NULL);
static DECLARE_WORK(wq_log_bottom_half_work, wq_log_bottom_half, NULL);
static DECLARE_WORK(wq_init_work, wq_init, NULL);
static DECLARE_WORK(wq_close_file_work, wq_close_file, NULL);
...
static char* get_privateram_address(void)
{
	if (ltt_lite_res.start && !ltt_lite_privateram_virtaddr) {
		ltt_lite_privateram_virtaddr = (char*)ioremap(ltt_lite_res.start,
...
static void __init ltt_lite_parser_mem(char **arg)
{
	...
	printk(KERN_ERR "ltt_lite_parser_mem:base 0x%lx, %lu\n", ltt_lite_res.start, size);
}
...
static int process_name_table_init(void)
{
	...
	while ((p = next_task(p)) != &init_task) {
		...
		do {
			...
			memcpy(process_log.comm, t->comm, TASK_COMM_LEN);
		...
		const char *buffer, size_t count, loff_t *data)
{
	...
	if (buf < 0 || buf > (LTT_LITE_MODE_SCHD | LTT_LITE_MODE_SYSC |
		LTT_LITE_MODE_MEM | LTT_LITE_MODE_TRAP | LTT_LITE_MODE_INT
		| LTT_LITE_MODE_SOFTIRQ | LTT_LITE_MODE_SIG |
		LTT_LITE_MODE_TMR))
		return count;
...
static void log_mem_status(unsigned long ignore)
{
	...
	while ((p = next_task(p)) != &init_task) {
		...
		mem_log.vmrss = mm->rss;
		mem_log.vmshr = mm->rss - mm->anon_rss;
...
/* Comment Following code to avoid conflict with the use of private ram driver */
/*
 *#ifdef CONFIG_MOT_FEAT_PRIVATE_RAM
 *	extern int privateram_get_partition_info(char*, unsigned long*, unsigned int*);
 *	if (privateram_get_partition_info("lttlite", &ltt_lite_res.start, 
 *		(unsigned int*)&ltt_lite_privateram_size)) {
 *		ltt_lite_res.start = 0;
 *		ltt_lite_privateram_size = 0;
 *		printk(KERN_INFO "ltt_lite privateram is disabled\n");
 *	} else {
 *		printk(KERN_INFO "ltt_lite privateram start at 0x%lx, size 0x%lx\n", 
 *			ltt_lite_res.start, ltt_lite_privateram_size);
 *	}
 *#endif
*/
...
void ltt_lite_ev_process_exit( void )
{
	...
	memcpy(process_log.comm, current->comm, TASK_COMM_LEN);
	commit_log(&process_log, sizeof(process_log), LTT_LITE_EV_PROCESS);
}

/*
 * get a time stamp
 * use jiffies + GPTCNT to get the time stamp
 * We can't do as ltt_lite_get_ms_time() does, as we may
 * be not able to obtain xtime_lock when returning from vst mode
 */
static inline void get_time_stamp(struct timeval *ktv)
{
	ktv->tv_sec = jiffies;
	ktv->tv_usec = __raw_readl(MXC_GPT_GPTCNT);
}
...
int ltt_lite_get_ms_time(struct timeval *ktv)
{
	...
	do {
		...
		jiffies_tmp = jiffies;
		jiffies_tmp = (__u32)(TIME_DELTA(jiffies_tmp, INITIAL_JIFFIES)
			/ HZ * 100 + TIME_DELTA(jiffies_tmp, INITIAL_JIFFIES)
			% HZ * 100 / HZ);

		ktv->tv_usec = system_timer->offset();
		ktv->tv_usec += jiffies_tmp % HZ * 10000;

		ktv->tv_sec = jiffies_tmp / HZ;
...
void ltt_lite_ev_log_process(int type, struct task_struct* p)
{
	...
	memcpy(process_log.comm, p->comm, TASK_COMM_LEN);
	commit_log(&process_log, sizeof(process_log), LTT_LITE_EV_PROCESS);
}
