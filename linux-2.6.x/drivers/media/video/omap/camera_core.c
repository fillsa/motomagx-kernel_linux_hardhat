...
static void camera_core_queue_dummy(struct camera_device *cam);
		...
		** of the frame has been completed */
		if (sgdma->next_sglist == sgdma->sglen) {
			...
			if (callback) {
				...
				/* Start filling dummy buffer if no other
				 * buffers queued. */
				if (NUM_SG_DMA == cam->free_sgdma)
                	        {
					camera_core_queue_dummy(cam);
	                        }
	...
	switch (cmd) {
		...
		case VIDIOC_STREAMOFF:
		{
			...
			for (i = 0; i < VIDEO_MAX_FRAME; i++) {
				if (NULL == q->bufs[i])
					continue;
			       	while (q->bufs[i]->state == STATE_QUEUED){schedule();}
			}
...
/* This dummy's callback does nothing. It is used only to siplify code which
 * calls camera_core_queue_dummy() */
static void
camera_core_dummy_callback(void *arg1, void *arg){;}

static void
camera_core_queue_dummy(struct camera_device *cam)
{
	if (!(cam->reading || cam->streaming) || !cam->capture_base )
	{
		/* We won't queue buffer if nobody reading data or if
		 * there is no buffer allocated */
		return;
	}

	sg_dma_address(&cam->capture_sglist) = cam->capture_base_phys;
	sg_dma_len(&cam->capture_sglist)= cam->pix.sizeimage;
	camera_core_sgdma_queue(cam, &cam->capture_sglist, 1, camera_core_dummy_callback, NULL);
}
...
camera_core_open(struct inode *inode, struct file *file)
{
	...
	videobuf_queue_init(&fh->vbq, &cam->vbq_ops, NULL, &cam->vbq_lock,
		fh->type, V4L2_FIELD_NONE, sizeof(struct videobuf_buffer), cam);
...
static void camera_core_platform_release(struct device *device)
{
}
...
static struct platform_device camera_core_device = {
	.name	= CAM_NAME,
	.dev	= {
			.release 	= camera_core_platform_release,
		  },
	.id	= 0,
};
