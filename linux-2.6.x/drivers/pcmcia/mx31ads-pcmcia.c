...
static int mx31ads_pcmcia_find_bsize(unsigned long win_size)
{
	...
	PCMCIA_DB("nr = %d bsize = 0x%0x\n", nr, bsize);
	if (bsize < 0 || i > nr) {
		printk("No such bsize\n");
		return -ENODEV;
	}
				 ...
				 socket_state_t * state)
{
	...
	if (ret < 0)
		printk(KERN_ERR "mx31ads_common_pcmcia: unable to configure"
		       " socket\n");
 ...
 * Returns: 0
 */
static int mx31ads_common_pcmcia_get_socket(struct pcmcia_socket *sock,
					    socket_state_t * state)
{
	struct mx31ads_pcmcia_socket *skt = to_mx31ads_pcmcia_socket(sock);

	*state = skt->cs_state;
	PCMCIA_DB("state->flags = %x, state->csc_mask = %x, state->Vcc = %d\n"
		  state->flags, state->csc_mask, state->Vcc);
					    ...
					    socket_state_t * state)
{
	...
	PCMCIA_DB("mask: %s%s%s%s%s%sflags: %s%s%s%s%s%sVcc %d Vpp %d irq %d\n",
	...
	PCMCIA_DB("csc_mask: %08x flags: %08x Vcc: %d Vpp: %d io_irq: %d\n",
...
static int mx31ads_pcmcia_set_window_reg(ulong start, ulong end, u_int window)
{
	...
	if (bsize < 0) {
		printk("Cannot set the window register\n");
		return -1;
	}
	/* Disable the window */
	...
	default:
		printk("Window %d is not support\n", window);
		return -1;
	}
				 ...
				 struct pccard_io_map *map)
{
	...
	PCMCIA_DB("map %u  speed %u start 0x%08x stop 0x%08x\n",
		  map->map, map->speed, map->start, map->stop);
	PCMCIA_DB("flags: %s%s%s%s%s%s%s%s\n",
		  (map->flags == 0) ? "<NONE>" : "",
	...
	PCMCIA_DB(KERN_ERR "IO window: _reg_PCMCIA_PBR(%d) = %08x\n",
		  IO_WINDOW, _reg_PCMCIA_PBR(IO_WINDOW));
	PCMCIA_DB(KERN_ERR "IO window: _reg_PCMCIA_POR(%d) = %08x\n",
				  ...
				  struct pccard_mem_map *map)
{
	...
	if (map->flags & MAP_ATTRIB) {
		...
		PCMCIA_DB("Attr window: _reg_PCMCIA_PBR(%d) = %08x\n",
			  ATTRIBUTE_MEMORY_WINDOW,
			  _reg_PCMCIA_PBR(ATTRIBUTE_MEMORY_WINDOW));
		PCMCIA_DB("_reg_PCMCIA_POR(%d) = %08x\n",
			  ATTRIBUTE_MEMORY_WINDOW,
		...
		PCMCIA_DB("Com window: _reg_PCMCIA_PBR(%d) = %08x\n",
			  COMMON_MEMORY_WINDOW,
			  _reg_PCMCIA_PBR(COMMON_MEMORY_WINDOW));
		PCMCIA_DB("Com window: _reg_PCMCIA_POR(%d) = %08x\n",
			  COMMON_MEMORY_WINDOW,
...
static struct pccard_operations mx31ads_common_pcmcia_operations = {
	...
	.get_socket = mx31ads_common_pcmcia_get_socket,
			    ...
			    struct pcmcia_state *state)
{
	...
	PCMCIA_DB("_reg_PCMCIA_PIPR = 0x%08x\n", pins);
...
static __inline__ void mx31ads_pcmcia_soft_reset(void)
{
	...
	msleep(2);
	PCMCIA_DB("_reg_PCMCIA_PGCR = %08x\n", _reg_PCMCIA_PGCR);
}
				...
				const socket_state_t * state)
{
	int ret = 0;
#ifdef PCMCIA_DEBUG
	int pins = _reg_PCMCIA_PIPR;
#endif

	if (state->Vcc != 0 && state->Vcc != 33 && state->Vcc != 50) {
		printk(KERN_ERR "mx31ads-pcmcia: unrecognized Vcc %d\n",
		       state->Vcc);
		return -1;
	}

	PCMCIA_DB("PIPR = %x, desired Vcc = %d.%dV\n", pins, state->Vcc / 10,
		  state->Vcc % 10);

	if (!(skt->socket.state & SOCKET_PRESENT) && (skt->pre_stat == 1)) {
		printk("Socket enter low power mode\n");
		skt->pre_stat = 0;
...
static int mx31ads_pcmcia_set_timing(struct mx31ads_pcmcia_socket *skt)
{
	...
	PCMCIA_DB("timing: io %d, mem %d, attr %d\n", timing.io, timing.mem,
		  timing.attr);
...
static void mx31ads_common_check_status(struct mx31ads_pcmcia_socket *skt)
{
	...
	PCMCIA_DB("entering PCMCIA monitoring thread\n");
...
static int mx31ads_common_drv_pcmcia_probe(struct device *dev,
					   struct pcmcia_low_level *ops)
{
	...
	printk(KERN_INFO "PCMCIA: Voltage the card supports: %d.%dV\n",
	       value / 10, value % 10);

	add_timer(&skt->poll_timer);

	if (!class_device_create_file(&skt->socket.dev, &class_device_attr_status)) 
		goto out_err_8;	
...
static int mx31ads_drv_pcmcia_remove(struct device *dev)
{
	struct mx31ads_pcmcia_socket *skt = dev_get_drvdata(dev);
...
static int mx31ads_drv_pcmcia_probe(struct device *dev)
{
	if (!machine_is_mx31ads())
		return -ENODEV;

	return mx31ads_common_drv_pcmcia_probe(dev, &mx31ads_pcmcia_ops);
}

static int mx31ads_drv_pcmcia_suspend(struct device *dev, u32 state, u32 level)
{
	int ret = 0;
...
static int mx31ads_drv_pcmcia_resume(struct device *dev, u32 level)
{
	int ret = 0;
 ...
 * Low level functions
 */
static struct device_driver mx31ads_pcmcia_driver = {
...
static int __init mx31ads_pcmcia_init(void)
{
	...
	if ((ret = driver_register(&mx31ads_pcmcia_driver)))
		return ret;
	printk("PCMCIA: Initialize i.Mx31 pcmcia socket\n");
...
{
	platform_device_unregister(&mx31ads_device);

	driver_unregister(&mx31ads_pcmcia_driver);
}
