...
PMIC_STATUS pmic_power_off(void)
{
	...
	CHECK_ERROR(pmic_write_reg(PRIO_PWM, REG_POWER_CONTROL_0, value, mask));
...
PMIC_STATUS pmic_power_set_pc_config(t_pc_config * pc_config)
{
	...
	CHECK_ERROR(pmic_write_reg(PRIO_PWM, REG_POWER_CONTROL_0,
				   pwrctrl_val_reg0, pwrctrl_mask_reg0));
	CHECK_ERROR(pmic_write_reg(PRIO_PWM, REG_POWER_CONTROL_1,
...
PMIC_STATUS pmic_power_get_pc_config(t_pc_config * pc_config)
{
	...
	CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_POWER_CONTROL_0,
				  &pwrctrl_val_reg0, PMIC_ALL_BITS));
	CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_POWER_CONTROL_1,
...
PMIC_STATUS pmic_power_regulator_on(t_pmic_regulator regulator)
{
	...
	CHECK_ERROR(pmic_write_reg(PRIO_PWM, reg, reg_val, reg_mask));
					     ...
					     t_regulator_voltage * voltage)
{
	...
	if (regulator == SW_SW1A) {
		CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_SWITCHERS_0,
					  &reg_val, PMIC_ALL_BITS));
	} else if (regulator == SW_SW1B) {
		CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_SWITCHERS_1,
					  &reg_val, PMIC_ALL_BITS));
	} else if (regulator == SW_SW2A) {
		CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_SWITCHERS_2,
					  &reg_val, PMIC_ALL_BITS));
	} else if (regulator == SW_SW2B) {
		CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_SWITCHERS_3,
					  &reg_val, PMIC_ALL_BITS));
	} else if (regulator == SW_SW3) {
		CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_SWITCHERS_5,
					  &reg_val, PMIC_ALL_BITS));
		...
		CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_REGULATOR_SETTING_0,
					  &reg_val, PMIC_ALL_BITS));
	} else if ((regulator == REGU_VVIB) || (regulator == REGU_VRF1) ||
		   (regulator == REGU_VRF2) ||
		   (regulator == REGU_VMMC1) || (regulator == REGU_VMMC2)) {
		CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_REGULATOR_SETTING_1,
					  &reg_val, PMIC_ALL_BITS));
	}
					...
					t_regulator_voltage * dvs)
{
	...
	CHECK_ERROR(pmic_read_reg(PRIO_PWM, reg, &reg_val, reg_mask));
					   ...
					   t_switcher_factor factor)
{
	...
	CHECK_ERROR(pmic_write_reg(PRIO_PWM, REG_SWITCHERS_4,
				   reg_val, reg_mask));
					   ...
					   t_switcher_factor * factor)
{
	...
	CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_SWITCHERS_4,
				  &reg_val, reg_mask));
...
{
	pmic_power_regulator_set_voltage(regulator, config->voltage);

	if ((regulator == SW_SW1A) || (regulator == SW_SW1B) ||
	    (regulator == SW_SW2A) || (regulator == SW_SW2B)) {
		CHECK_ERROR(pmic_power_switcher_set_dvs(regulator,
							config->voltage_lvs));
		CHECK_ERROR(pmic_power_switcher_set_stby(regulator,
							 config->voltage_stby));
		CHECK_ERROR(pmic_power_switcher_set_mode(regulator,
							 config->mode, false));
		CHECK_ERROR(pmic_power_switcher_set_mode(regulator,
							 config->stby_mode,
							 true));
	...
	} else if (regulator == SW_PLL) {
		CHECK_ERROR(pmic_power_switcher_set_factor(regulator,
							   config->factor));
	} else if (regulator != REGU_VVIB) {
		CHECK_ERROR(pmic_power_regulator_set_lp_mode(regulator,
							     config->lp_mode));
	} else {
		return PMIC_PARAMETER_ERROR;
	}
...
PMIC_STATUS pmic_power_regulator_get_config(t_pmic_regulator regulator,
					    t_regulator_config * config)
{
	pmic_power_regulator_get_voltage(regulator, &config->voltage);

	if ((regulator == SW_SW1A) || (regulator == SW_SW1B) ||
	    (regulator == SW_SW2A) || (regulator == SW_SW2B)) {
		CHECK_ERROR(pmic_power_switcher_get_dvs(regulator,
							&config->voltage_lvs));
		CHECK_ERROR(pmic_power_switcher_get_stby(regulator,
							 &config->
							 voltage_stby));
		...
		CHECK_ERROR(pmic_power_switcher_get_factor(regulator,
							   &config->factor));
	} else if (regulator != REGU_VVIB) {
		CHECK_ERROR(pmic_power_regulator_get_lp_mode(regulator,
							     &config->lp_mode));
	}
...
PMIC_STATUS pmic_power_vib_pin_en(bool en)
{
	...
	if (mc13783_ver >= 20) {
		...
		CHECK_ERROR(pmic_write_reg(PRIO_PWM, REG_POWER_MISCELLANEOUS,
					   reg_val, reg_mask));
		return PMIC_SUCCESS;
	} else {
		return PMIC_NOT_SUPPORTED;
...
PMIC_STATUS pmic_power_gets_vib_pin_state(bool * en)
{
	...
	if (mc13783_ver >= 20) {
		reg_mask = BITFMASK(MC13783_REGCTRL_VIBPINCTRL);
		CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_POWER_MISCELLANEOUS,
					  &reg_val, reg_mask));
...
PMIC_STATUS pmic_power_get_power_mode_sense(struct t_p_up_sense * p_up_sense)
{
	...
	CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_POWER_UP_MODE_SENSE,
...
PMIC_STATUS pmic_power_set_regen_assig(t_pmic_regulator regulator, bool en_dis)
{
	...
	CHECK_ERROR(pmic_write_reg(PRIO_PWM, REG_REGEN_ASSIGNMENT,
				   reg_val, reg_mask));
				       ...
				       bool * en_dis)
{
	...
	CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_REGEN_ASSIGNMENT,
				  &reg_val, reg_mask));
...
PMIC_STATUS pmic_power_set_auto_reset_en(bool en)
{
	...
	CHECK_ERROR(pmic_write_reg(PRIO_PWM, REG_POWER_CONTROL_2,
				   reg_val, reg_mask));
	return PMIC_SUCCESS;
}
...
PMIC_STATUS pmic_power_get_auto_reset_en(bool * en)
{
	...
	reg_mask = BITFMASK(MC13783_AUTO_RESTART);
	CHECK_ERROR(pmic_read_reg(PRIO_PWM, REG_POWER_CONTROL_2,
				  &reg_val, reg_mask));
...
PMIC_STATUS pmic_power_event(t_pwr_int event, void *callback, bool sub)
{
	...
	if (sub == true) {
		CHECK_ERROR(pmic_event_subscribe(&power_event));
	} else {
		CHECK_ERROR(pmic_event_unsubscribe(&power_event));
	}
	return PMIC_SUCCESS;
}
			    ...
			    unsigned int cmd, unsigned long arg)
{
	...
	case PMIC_REGULATOR_SET_CONFIG:
		...
		CHECK_ERROR(pmic_power_regulator_set_config
			    (cfg_param->regulator, &cfg_param->cfg));
	...
	case PMIC_REGULATOR_GET_CONFIG:
		...
		CHECK_ERROR(pmic_power_regulator_get_config
			    (cfg_param->regulator, &cfg_param->cfg));
...
static int __init pmic_power_init(void)
{
	...
	if (pmic_power_major < 0) {
		TRACEMSG_POWER(_K_D("Unable to get a major for pmic_power"));
		return -1;
	}
	...
	ret = driver_register(&pmic_power_driver_ldm);
	if (ret == 0) {
		ret = platform_device_register(&pmic_power_ldm);
		if (ret != 0) {
			driver_unregister(&pmic_power_driver_ldm);
		}
	}

	printk(KERN_INFO "Pmic Power loading\n");
...
static void __exit pmic_power_exit(void)
{
	...
	TRACEMSG_POWER(_K_I("Pmic_power : successfully unloaded"));
}
