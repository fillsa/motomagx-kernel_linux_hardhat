 ...
 *
 * @ingroup PMIC_MC13783_AUDIO - VVV
 */
				    ...
				    const PMIC_AUDIO_NUMSLOTS numSlots)
{
	...
	const unsigned int VCODEC_MASK = SET_BITS(regAUD_CODEC, CDCSSISEL, 1) |
	    SET_BITS(regAUD_CODEC, CDCFS, 3) | SET_BITS(regAUD_CODEC, CDCSM, 1);

	const unsigned int SSI_NW_MASK = SET_BITS(regSSI_NETWORK, STDCSLOTS, 1);
	...
	if (handle == (PMIC_AUDIO_HANDLE *) NULL) {
		...
		if ((handle == vCodec.handle) &&
			...
			if ((stDAC.handleState == HANDLE_IN_USE) &&
			    (stDAC.busID == busID)) {
				TRACEMSG_AUDIO(_K_D
					       ("The requested bus already in USE\n"));
				rc = PMIC_PARAMETER_ERROR;
				...
				if (pmic_write_reg
				    (PRIO_AUDIO, REG_AUDIO_CODEC, reg_value,
			...
			if ((vCodec.handleState == HANDLE_IN_USE) &&
				...
				if (pmic_write_reg
				    (PRIO_AUDIO, REG_AUDIO_STEREO_DAC,
					...
					if (pmic_write_reg
					    (PRIO_AUDIO, REG_AUDIO_SSI_NETWORK,
					     reg_value,
			...
			TRACEMSG_AUDIO(_K_D("Handles only STDAC and VCODEC\n"));
		}
...
PMIC_STATUS pmic_audio_enable(const PMIC_AUDIO_HANDLE handle)
{
	...
	if ((handle == stDAC.handle) && (stDAC.handleState == HANDLE_IN_USE)) {
		...
		pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_0, AUDIO_BIAS_ENABLE,
			       AUDIO_BIAS_ENABLE);
		reg_mask = SET_BITS(regAUDIO_RX_0, HSDETEN, 1) |
		    SET_BITS(regAUDIO_RX_0, HSDETAUTOB, 1);
		reg_write = SET_BITS(regAUDIO_RX_0, HSDETEN, 1) |
		    SET_BITS(regAUDIO_RX_0, HSDETAUTOB, 1);
		rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_0, reg_write,
				    reg_mask);
		if (rc == PMIC_SUCCESS) {
		}
		/* Then we can enable the Stereo DAC. */
		rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_STEREO_DAC,
				    STDAC_ENABLE, STDAC_ENABLE);
		/*pmic_read_reg(PRIO_AUDIO, REG_AUDIO_STEREO_DAC, &reg_value); */
		if (rc != PMIC_SUCCESS) {
			TRACEMSG_AUDIO(_K_D
				       ("Failed to enable the Stereo DAC\n"));
			rc = PMIC_ERROR;
		}
	} else if ((handle == vCodec.handle)
		   && (vCodec.handleState == HANDLE_IN_USE)) {
		...
		rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_CODEC, VCODEC_ENABLE,
				    VCODEC_ENABLE);

		/* pmic_read_reg(PRIO_AUDIO, REG_AUDIO_CODEC, &reg_value); */
		if (rc != PMIC_SUCCESS) {
			TRACEMSG_AUDIO(_K_D
				       ("Failed to enable the Voice codec\n"));
			rc = PMIC_ERROR;
		}
	}
	/* Exit critical section. */
...
PMIC_STATUS pmic_audio_disable(const PMIC_AUDIO_HANDLE handle)
{
	...
	down_interruptible(&mutex);
	if ((handle == stDAC.handle) && (stDAC.handleState == HANDLE_IN_USE)) {
		rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_STEREO_DAC, 0,
				    STDAC_DISABLE);
	} else if ((handle == vCodec.handle)
		   && (vCodec.handleState == HANDLE_IN_USE)) {
		rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_CODEC, 0,
				    VCODEC_DISABLE);
	}
	if (rc == PMIC_SUCCESS) {
		TRACEMSG_AUDIO(_K_D("Disabled successfully\n"));
	}
	/* Exit critical section. */
	up(&mutex);
	return rc;
...
PMIC_STATUS pmic_audio_reset_all(void)
{
	...
	if (pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_1, RESET_AUDIO_RX_1,
			   PMIC_ALL_BITS) != PMIC_SUCCESS) {
		rc = PMIC_ERROR;
	} else {
		audio_rx1_reset = 1;
	}
	if (pmic_write_reg(PRIO_AUDIO, REG_AUDIO_SSI_NETWORK, RESET_SSI_NETWORK,
			   PMIC_ALL_BITS) != PMIC_SUCCESS) {
		rc = PMIC_ERROR;
	} else {
		audio_ssi_reset = 1;
	...
	if (pmic_write_reg
	    (PRIO_AUDIO, REG_AUDIO_STEREO_DAC, RESET_ST_DAC,
	     PMIC_ALL_BITS) != PMIC_SUCCESS) {
		rc = PMIC_ERROR;
	} else {
	...
	if (pmic_write_reg(PRIO_AUDIO, REG_AUDIO_CODEC, RESET_AUD_CODEC,
			   PMIC_ALL_BITS) != PMIC_SUCCESS) {
		rc = PMIC_ERROR;
	} else {
		/* Also reset the driver state information to match. Note that we
	...
	if (pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_0, RESET_AUDIO_RX_0,
			   PMIC_ALL_BITS) != PMIC_SUCCESS) {
		rc = PMIC_ERROR;
	} else {
	...
	if (pmic_write_reg(PRIO_AUDIO, REG_AUDIO_TX, RESET_AUDIO_TX,
			   PMIC_ALL_BITS) != PMIC_SUCCESS) {
		rc = PMIC_ERROR;
	} else {
		/* Also reset the driver state information to match. Note that we
				      ...
				      rampSpeed)
{
	...
	rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_0, reg_value, reg_mask);
	return rc;
}
...
PMIC_STATUS pmic_audio_antipop_disable(void)
{
	 ...
	 */
	rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_0, reg_write, reg_mask);
...
PMIC_STATUS pmic_audio_digital_filter_reset(const PMIC_AUDIO_HANDLE handle)
{
	...
	if ((handle == stDAC.handle) && (stDAC.handleState == HANDLE_IN_USE)) {
		...
		if (pmic_write_reg(PRIO_AUDIO, REG_AUDIO_STEREO_DAC, 1,
				   reg_mask) != PMIC_SUCCESS) {
			rc = PMIC_ERROR;
		} else {
			TRACEMSG_AUDIO(_K_D("STDAC filter reset\n"));
		}

	} else if ((handle == vCodec.handle) &&
		   (vCodec.handleState == HANDLE_IN_USE)) {
		reg_mask = SET_BITS(regAUD_CODEC, CDCRESET, 1);
		if (pmic_write_reg(PRIO_AUDIO, REG_AUDIO_CODEC, 1,
				   reg_mask) != PMIC_SUCCESS) {
			rc = PMIC_ERROR;
		} else {
			TRACEMSG_AUDIO(_K_D("CODEC filter reset\n"));
		}
	}
	return rc;
}
...
void pmic_audio_dump_registers(void)
{
	...
	if (pmic_read_reg(PRIO_AUDIO, REG_AUDIO_CODEC, &reg_value, REG_FULLMASK)
	    == PMIC_SUCCESS) {
		TRACEMSG_ALL_TIME(KERN_INFO "Audio Codec = 0x%x\n", reg_value);
	} else {
		TRACEMSG_AUDIO(_K_D("Failed to read audio codec\n"));
	}

	/* Dump the ST DAC (Stereo DAC) register. */
	if (pmic_read_reg
	    (PRIO_AUDIO, REG_AUDIO_STEREO_DAC, &reg_value,
	     REG_FULLMASK) == PMIC_SUCCESS) {
		TRACEMSG_ALL_TIME(KERN_INFO "Stereo DAC = 0x%x\n", reg_value);
	} else {
		TRACEMSG_AUDIO(_K_D("Failed to read Stereo DAC\n"));
	}

	/* Dump the SSI NW register. */
	if (pmic_read_reg
	    (PRIO_AUDIO, REG_AUDIO_SSI_NETWORK, &reg_value,
	     REG_FULLMASK) == PMIC_SUCCESS) {
		TRACEMSG_ALL_TIME(KERN_INFO "SSI Network = 0x%x\n", reg_value);
	} else {
		TRACEMSG_AUDIO(_K_D("Failed to read SSI network\n"));
	}

	/* Dump the Audio RX 0 register. */
	if (pmic_read_reg(PRIO_AUDIO, REG_AUDIO_RX_0, &reg_value, REG_FULLMASK)
	    == PMIC_SUCCESS) {
		TRACEMSG_ALL_TIME(KERN_INFO "Audio RX 0 = 0x%x\n", reg_value);
	} else {
		TRACEMSG_AUDIO(_K_D("Failed to read audio RX 0\n"));
	}

	/* Dump the Audio RX 1 register. */
	if (pmic_read_reg(PRIO_AUDIO, REG_AUDIO_RX_1, &reg_value, REG_FULLMASK)
	    == PMIC_SUCCESS) {
		TRACEMSG_ALL_TIME(KERN_INFO "Audio RX 1 = 0x%x\n", reg_value);
	} else {
		TRACEMSG_AUDIO(_K_D("Failed to read audio RX 1\n"));
	}
	/* Dump the Audio TX register. */
	if (pmic_read_reg(PRIO_AUDIO, REG_AUDIO_TX, &reg_value, REG_FULLMASK) ==
	    PMIC_SUCCESS) {
		TRACEMSG_ALL_TIME(KERN_INFO "Audio Tx = 0x%x\n", reg_value);
	} else {
		TRACEMSG_AUDIO(_K_D("Failed to read audio TX\n"));
	}
	...
	if (handle == (PMIC_AUDIO_HANDLE *) NULL) {
		...
		if ((clockIn != CLOCK_IN_CLIA) && (clockIn != CLOCK_IN_CLIB)) {
			...
			if (pmic_write_reg
				...
				TRACEMSG_AUDIO(_K_D("CODEC clock set\n"));
						...
						timeslot)
{
	...
	if ((handle == vCodec.handle) &&
		...
		rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_SSI_NETWORK,
				    reg_write, reg_mask);
					 ...
					 const PMIC_AUDIO_VCODEC_CONFIG config)
{
	...
	if ((handle == vCodec.handle) && (vCodec.handleState == HANDLE_IN_USE)) {
		...
		if (reg_mask == 0) {
			...
			rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_CODEC,
					    reg_write, reg_mask);
		}
...
PMIC_STATUS pmic_audio_vcodec_enable_bypass(const PMIC_AUDIO_HANDLE handle)
{
	...
	if ((handle == vCodec.handle) && (vCodec.handleState == HANDLE_IN_USE)) {
		rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_CODEC, reg_write,
				    reg_mask);
	}
		...
		/*
		   ...
		   rc = PMIC_PARAMETER_ERROR;
			...
			if (pmic_write_reg
				...
				TRACEMSG_AUDIO(_K_D("STDAC clock set\n"));
				rc = PMIC_SUCCESS;
					       ...
					       timeslot)
{
	...
	if ((handle == stDAC.handle) && (stDAC.handleState == HANDLE_IN_USE)) {
		if ((timeslot == USE_TS0_TS1) || (timeslot == USE_TS2_TS3)
			...
			if (pmic_write_reg
			    (PRIO_AUDIO, REG_AUDIO_SSI_NETWORK, timeslot,
			     reg_mask) != PMIC_SUCCESS) {
				rc = PMIC_ERROR;
			} else {
				TRACEMSG_AUDIO(_K_D
					       ("STDAC primary timeslot set\n"));
					...
					const PMIC_AUDIO_STDAC_CONFIG config)
{
	...
	if ((handle == stDAC.handle) && (stDAC.handleState == HANDLE_IN_USE)) {
		...
		rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_STEREO_DAC, reg_write,
				    reg_mask);

		if (rc == PMIC_SUCCESS) {
			stDAC.config |= config;
			TRACEMSG_AUDIO(_K_D("STDAC config set\n"));
	...
	if ((handle == vCodec.handle) && (vCodec.handleState == HANDLE_IN_USE)) {
		if (!((leftChannel == NO_MIC) || (leftChannel == MIC1_LEFT))) {
			...
			if (reg_mask == 0) {
				...
				rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_TX,
						    reg_write, reg_mask);
					      ...
					      rightChannelGain)
{
	...
	if ((handle == vCodec.handle) && (vCodec.handleState == HANDLE_IN_USE)) {
		if (!(((leftChannelGain >= MIC_GAIN_MINUS_8DB)
			...
			rc = PMIC_PARAMETER_ERROR;
			TRACEMSG_AUDIO(_K_D
				       ("VCODEC set record gain - wrong gain value\n"));
			...
			if (vCodec.rightChannelMic.mic == MIC1_RIGHT_MIC_MONO) {
				...
				reg_mask |= SET_BITS(regAUDIO_TX, AMC2ITOV, 1) |
				    SET_BITS(regAUDIO_TX, PGATXR, 32);
				if (rightChannelMode == VOLTAGE_TO_VOLTAGE) {
					reg_write |=
				...
				reg_mask |= SET_BITS(regAUDIO_TX, PGATXR, 32);
					     ...
					     biasCircuit)
{
	...
	if ((handle == vCodec.handle) && (vCodec.handleState == HANDLE_IN_USE)) {
		...
		if (reg_mask != 0) {
			rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_TX, reg_write,
					    reg_mask);
		}
	}
					   ...
					   gainOut)
{
	...
	if ((handle == vCodec.handle) && (vCodec.handleState == HANDLE_IN_USE)) {
		if (!((rxSecondaryTimeslot >= USE_TS0)
			...
			TRACEMSG_AUDIO(_K_D
				       ("VCODEC enable mixer - wrong mix in gain\n"));

		} else if (!((gainOut >= VCODEC_MIX_OUT_0DB)
			     && (gainOut <= VCODEC_MIX_OUT_MINUS_6DB))) {
			TRACEMSG_AUDIO(_K_D
				       ("VCODEC enable mixer - wrong mix out gain\n"));
		} else {
			...
			rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_SSI_NETWORK,
					    reg_write, reg_mask);
			if (rc == PMIC_SUCCESS) {
				TRACEMSG_AUDIO(_K_D("Vcodec mixer enabled\n"));
			}
		}
	}
...
PMIC_STATUS pmic_audio_vcodec_disable_mixer(const PMIC_AUDIO_HANDLE handle)
{
	...
	if ((handle == vCodec.handle) && (vCodec.handleState == HANDLE_IN_USE)) {
		...
		if (rc == PMIC_SUCCESS) {
			TRACEMSG_AUDIO(_K_D("Vcodec mixer disabled\n"));
		}
	...
	if ((handle == stDAC.handle) && (stDAC.handleState == HANDLE_IN_USE)) {
		if (!((rxSecondaryTimeslot >= USE_TS0_TS1)
			...
			rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_SSI_NETWORK,
					    reg_write, reg_mask);
			if (rc == PMIC_SUCCESS) {
				TRACEMSG_AUDIO(_K_D("STDAC mixer enabled\n"));
			}
		}
				       ...
				       const PMIC_AUDIO_OUTPUT_PORT port)
{
	...
	if ((port == MONO_ALERT) || (port == MONO_EXTOUT)) {
		...
		if (reg_mask == 0) {
			...
			rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_0,
					    reg_write, reg_mask);

			if (rc == PMIC_SUCCESS) {
				TRACEMSG_AUDIO(_K_D("output ports enabled\n"));
	...
	if ((port == MONO_ALERT) || (port == MONO_EXTOUT)) {
		...
		if (reg_mask == 0) {
			...
			rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_0,
					    reg_write, reg_mask);
			if (rc == PMIC_SUCCESS) {
				TRACEMSG_AUDIO(_K_D("output ports disabled\n"));
				audioOutput.outputPort &= ~port;
			}
		}
	}
						 ...
						 gain)
{
	...
	if ((gain == STEREO_IN_GAIN_0DB) || (gain == STEREO_IN_GAIN_PLUS_18DB)) {
		if ((handle == extStereoIn.handle) &&
			...
			rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_1,
					    reg_write, reg_mask);

			if (rc == PMIC_SUCCESS) {
				TRACEMSG_AUDIO(_K_D("Ext stereo gain set\n"));
					  ...
					  const PMIC_AUDIO_OUTPUT_PGA_GAIN gain)
{
	...
	if (!((gain >= OUTPGA_GAIN_MINUS_33DB)
		...
		rc = PMIC_NOT_SUPPORTED;
		TRACEMSG_AUDIO(_K_D
			       ("output set PGA gain - wrong gain value\n"));
	} else {
		if ((gain >= OUTPGA_GAIN_MINUS_33DB)
		    && (gain <= OUTPGA_GAIN_PLUS_6DB)) {
			reg_gain = gain + 2;
		...
		if (reg_mask == 0) {
			...
			if (rc == PMIC_SUCCESS) {
				TRACEMSG_AUDIO(_K_D("Output PGA gains set\n"));
...
PMIC_STATUS pmic_audio_output_enable_mixer(const PMIC_AUDIO_HANDLE handle)
{
	...
	if (reg_mask == 0) {
		...
		rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_1, reg_write,
				    reg_mask);
		...
		if (rc == PMIC_SUCCESS) {
			...
			if (rc == PMIC_SUCCESS) {
				TRACEMSG_AUDIO(_K_D
					       ("Output PGA mixers enabled\n"));
				rc = PMIC_SUCCESS;
			}

		} else {
			TRACEMSG_AUDIO(_K_D
				       ("Error writing mixer enable to register\n"));
		}
						    ...
						    handle)
{
	...
	if (((handle == stDAC.handle) &&
	    ...
	    ((handle == extStereoIn.handle) &&
	     (extStereoIn.handleState == HANDLE_IN_USE))) {
		rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_0, 0, reg_mask);
		if (rc == PMIC_SUCCESS) {
			TRACEMSG_AUDIO(_K_D("Phantom ground enabled\n"));
						     ...
						     handle)
{
	...
	if (((handle == stDAC.handle) &&
	    ...
	    ((handle == extStereoIn.handle) &&
	     (extStereoIn.handleState == HANDLE_IN_USE))) {
		rc = pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_0, 1, reg_mask);
		if (rc == PMIC_SUCCESS) {
			TRACEMSG_AUDIO(_K_D("Phantom ground disabled\n"));
...
static PMIC_STATUS pmic_audio_reset_device(const PMIC_AUDIO_HANDLE handle)
{
	...
	if ((handle == stDAC.handle) && (stDAC.handleState == HANDLE_IN_USE)) {
		...
		pmic_write_reg(PRIO_AUDIO, REG_AUDIO_TX, RESET_AUDIO_TX,
			       REG_FULLMASK);
		...
		reg_mask = SET_BITS(regAUDIO_RX_1, ARXINEN, 1);
		pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_1, 0, reg_mask);

		reg_mask = SET_BITS(regAUDIO_RX_0, ADDRXIN, 1);
		pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_0, 0, reg_mask);
...
static void pmic_audio_event_handler(void *param)
{
	/* Currently not supported by MC13783 driver */
}
...
{
	TRACEMSG_AUDIO(_K_D("mc13783 audio loading\n"));
...
{
	TRACEMSG_AUDIO(_K_D("mc13783 audio unloading\n"));
	 ...
	 */
	pmic_write_reg(PRIO_AUDIO, REG_AUDIO_STEREO_DAC, RESET_ST_DAC,
		       REG_FULLMASK);
	pmic_write_reg(PRIO_AUDIO, REG_AUDIO_CODEC, RESET_AUD_CODEC,
		       REG_FULLMASK);
	pmic_write_reg(PRIO_AUDIO, REG_AUDIO_TX, RESET_AUDIO_TX, REG_FULLMASK);
	pmic_write_reg(PRIO_AUDIO, REG_AUDIO_SSI_NETWORK, RESET_SSI_NETWORK,
		       REG_FULLMASK);
	pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_0, RESET_AUDIO_RX_0,
		       REG_FULLMASK);
	pmic_write_reg(PRIO_AUDIO, REG_AUDIO_RX_1, RESET_AUDIO_RX_1,
		       REG_FULLMASK);
}
