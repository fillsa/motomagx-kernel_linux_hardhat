...
hac_ret hac_suspend(struct device * dev, ulong state, ulong level)
{
	...
	hac_ret ret_val = HAC_SUCCESS;
	hac_ctl = __raw_readl(HAC_CTL);
	printk("HAC Module: HAC suspend function.%08lX\n", level);
	switch (level) {
	case SUSPEND_DISABLE:
		printk("HAC Module: In suspend Disable\n");
		/* Setting suspend flag. */
		hac_suspend_state = 1;
		break;

	case SUSPEND_SAVE_STATE:
		HAC_DEBUG("HAC Module: In Suspend save state.\n");
		/* Do Nothing. */
		break;
...
hac_ret hac_resume(struct device * dev, ulong level)
{
	...
	hac_ret ret_val = HAC_SUCCESS;
	hac_ctl = __raw_readl(HAC_CTL);
	printk("HAC Module: HAC resume function.%08lX\n", level);
	switch (level) {
	...
	case RESUME_RESTORE_STATE:
		HAC_DEBUG("HAC Module: Resume restore state.\n");
		/* Do Nothing. */
		break;

	case RESUME_ENABLE:
		printk("HAC Module: Resume Enable state called.\n");
