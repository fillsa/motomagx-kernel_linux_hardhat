...
*
* The macros #virt_to_bus() and #bus_to_virt() are used extensively.
*
 ...
 */
#define virt_to_bus(virt) ((virt)? __pa(virt) : 0)
/*! Convert physical address to virtual address
 * @param  phys    The physcial address to be converted.
 *
 * @return The associated virtual address
 */
#define bus_to_virt(phys) ((phys)? __va(phys) : 0)
#endif
...
static struct dma_pool* big_dma_pool;
static struct dma_pool* small_dma_pool;
#endif
    ...
    sah_Head_Desc *user_head_desc)
{
    ...
    do {
        ...
        if (is_this_the_head) {
            head_desc = (sah_Head_Desc*)sah_Alloc_Head_Descriptor();
            ...
            if( head_desc == NULL ) {
#ifdef DIAG_DRV_IF
                LOG_KDIAG("sah_Alloc_Head_Descriptor() failed.");
#endif
                drv_if_done = TRUE;
                ...
                result = copy_from_user (
                               ...
                               user_head_desc,
                               (sizeof(*head_desc) -
                                sizeof(head_desc->desc.dma_addr) -
                                sizeof(head_desc->desc.virt_addr) -
                                sizeof(head_desc->desc.original_ptr1) -
...
*/                                sizeof(head_desc->out1_ptr) -
                                sizeof(head_desc->out2_ptr) -
                                sizeof(head_desc->out_len)));
                ...
                if( result != 0 ) {
                     ...
                     */
                    head_desc->user_ref = (uint32_t)head_desc->desc.dma_addr;
                    head_desc->uco_flags =
                        (uint32_t)head_desc->desc.original_ptr1;
#ifdef DIAG_DRV_IF
                    sprintf(Diag_msg, "User flags: %x; User Reference: %x",
                            head_desc->uco_flags, head_desc->user_ref);
                    LOG_KDIAG(Diag_msg);
#endif
                     ...
                     * always be deleted */
                    head_desc->uco_flags &= ~FSL_UCO_SAVE_DESC_CHAIN;
            ...
            if( curr_desc == NULL ) {
                ...
                prev_desc->next = (sah_Desc *)curr_desc->dma_addr;
        ...
        if (status == FSL_RETURN_OK_S) {
            if (!(curr_desc->header & SAH_LLO_BIT)) {
                ...
                if (curr_desc->ptr1 != NULL) {
                    ...
                    curr_desc->original_ptr1 = sah_Copy_Links(curr_desc->ptr1);
                    if (curr_desc->original_ptr1 == NULL) {
                        ...
                        curr_desc->ptr2 = NULL;
#ifdef DIAG_DRV_IF
                        LOG_KDIAG("sah_Copy_Links() failed.");
#endif
                    } else {
                        curr_desc->ptr1 = (void*)
                            ((sah_Link*)curr_desc->original_ptr1)->dma_addr;
                    }
                }
                ...
                if (status == FSL_RETURN_OK_S) {
                    if (curr_desc->ptr2 != NULL) {
                        ...
                        curr_desc->original_ptr2 =
                            sah_Copy_Links(curr_desc->ptr2);
                        if (curr_desc->original_ptr2 == NULL) {
                            ...
                            curr_desc->ptr2 =
                                (void*) (((sah_Link*)curr_desc->original_ptr2)
                                         -> dma_addr);
                        }
                    }
                }
                 ...
                 */
#ifdef DIAG_DRV_IF
                LOG_KDIAG("The LLO bit in the Descriptor Header field was "
    ...
    sah_Link *first_link)
{
    ...
    while (link != NULL) {
        ...
        if (link->flags & SAH_STORED_KEY_INFO) {
            ...
            scc_status = scc_get_slot_info(link->ownerid,
                                           link->slot,
...
#if defined(SAH_COPY_KERNEL_DATA)
                {
                    /* new vptr */
                    link->data = dma_alloc_coherent(NULL,
                                                    sah_Link_Get_Len(link),
                                                    GFP_ATOMIC | __GFP_DMA);

                    if (link->data == NULL) {
                        /* Need to undo whatever has been done up to this
                           point. */
                        os_printk("Abort! Abort! Abort!\n");
                        break;
                    }

                    if (! (link->flags & SAH_OUTPUT_LINK)) {
                        /* copy input to Sahara */
                        memcpy(link->data, link->original_data,
                               sah_Link_Get_Len(link));

#ifdef DO_DBG
                        sah_Dump_Region("IData", link->data, link->len );
#endif
                        /* DEBUG ONLY !!! */
                    }
                }
#endif /* copy kernel data */

                /* All pointers are virtual right now */
                link->data = (void*)os_pa(link->data);
#ifdef DO_DBG
                os_printk("%sput: %p (%d)\n",
                          (link->flags & SAH_OUTPUT_LINK) ? "out" : "in",
                          link->data, link->len);
#endif

                if ( link->flags & SAH_OUTPUT_LINK) {
                    /* clean and invalidate */
                    os_cache_flush_range(link->original_data, link->len);
                } else {
                    os_cache_clean_range(link->original_data, link->len);
                }
            } /* not prephys */
        ...
        if (link->flags & SAH_OUTPUT_LINK) {
            if ((((uint32_t)link->data%32) != 0 )
                && (((uint32_t)link->data & 1023 ) + link->len) > 1024) {
                ...
                new_link->original_data = link->original_data + link->len;
    ...
    sah_Head_Desc *first_desc)
{
    ...
    if (! (first_desc->uco_flags & FSL_UCO_CHAIN_PREPHYSICALIZED)) {
        while (desc != NULL) {
            ...
            if (desc->ptr1 != NULL) {
                if ((desc->ptr1 = sah_Physicalise_Links(desc->ptr1)) == NULL) {
                    /* Clean up ...*/
                    sah_DePhysicalise_Descriptors(first_desc);
                    first_desc = NULL;
                    break;
                }
            }
            ...
            if (desc->ptr2 != NULL) {
                if ((desc->ptr2 = sah_Physicalise_Links(desc->ptr2)) == NULL) {
                    /* Clean up ...*/
                    sah_DePhysicalise_Descriptors(first_desc);
                    first_desc = NULL;
                    break;
    ...
    sah_Link *first_link)
{
    ...
    while (link != NULL) {
        ...
        if (! (link->flags & SAH_STORED_KEY_INFO)
            ...
            /* */
            if (link->flags & SAH_OUTPUT_LINK) {
#if !defined(SAH_COPY_DATA)
                os_cache_inv_range(link->original_data, link->len);
#endif
            }
    ...
    sah_Head_Desc *first_desc)
{
    ...
    if (! (first_desc->uco_flags & FSL_UCO_CHAIN_PREPHYSICALIZED)) {
        while (desc != NULL) {
            ...
            if (desc->ptr1 != NULL) {
                desc->ptr1 = sah_DePhysicalise_Links(desc->original_ptr1);
            }
            if (desc->ptr2 != NULL) {
                desc->ptr2 = sah_DePhysicalise_Links(desc->original_ptr2);
            }
            if (desc->next != NULL) {
...
#error This code does not work
                /* move this to user blocking / get results??? */
        if ((link->flags & SAH_OUTPUT_LINK)
            && !(link->flags & SAH_STORED_KEY_INFO)) {
            /* Copy modified data back to user buffer */
            copy_to_user(link->original_data, link->copy_data, link->len);
        }
#endif
    ...
    sah_Link *ptr)
{
    ...
    while (drv_if_done == FALSE) {
        ...
        if( status == FSL_RETURN_OK_S ) {
            ...
            new_head_link = sah_Make_Links (&link_copy, &new_tail_link);
            ...
            if (new_head_link == NULL) {
                ...
                if( is_this_the_head == TRUE ) {
                     ...
                     * to the current link. */
                    prev_tail_link->next = (void*)new_head_link->dma_addr;
                    prev_tail_link->original_next = new_head_link;
                }
            }
        }
    ...
    sah_Link **tail)
{
            ...
            if ((scc_status == SCC_RET_OK) && (ptr->len <= max_len)) {
                ...
                *tail = link;
                ...
                if (scc_status == SCC_RET_OK) {
                    LOG_KDIAG("SCC sah_Link key slot reference is too long");
                } else {
                    LOG_KDIAG("SCC sah_Link slot slot reference is invalid");
                }
#endif
                sah_Free_Link(link);
                status = FSL_RETURN_INTERNAL_ERROR_S;
    ...
    if (status == FSL_RETURN_OK_S) {
        ...
        local_pages = kmalloc(nr_pages * sizeof(struct page*), GFP_KERNEL);
        ...
        if (local_pages == NULL) {
            ...
            result = get_user_pages(current, current->mm,
                                    (unsigned long)(ptr->data), nr_pages,
                                    write, 0 /* noforce */, local_pages, NULL);
            ...
            if (result < nr_pages) {
                ...
                if (result > 0) {
                    for (page_index = 0; page_index < result; page_index++) {
                        page_cache_release(local_pages[page_index]);
                    }
                }
                status = FSL_RETURN_INTERNAL_ERROR_S;
            }
    ...
    LOG_KDIAG(Diag_msg);
            ...
            if (status == FSL_RETURN_OK_S) {
                if( is_this_the_head == TRUE ) {
                    ...
                    prev_link->next = (sah_Link *)link->dma_addr;
                }
                ...
                if (page_index == 0) {
                     ...
                     */
                    buffer_start += (unsigned long)sah_Link_Get_Data (ptr)
                                    & ~PAGE_MASK;
                    buffer_length = PAGE_SIZE
                                    - ((unsigned long)sah_Link_Get_Data (ptr)
                                       & ~PAGE_MASK);
                } else {
                    buffer_length = PAGE_SIZE;
                }
                ...
                if (page_index == nr_pages-1) {
                     ...
                     */
                    buffer_length -=
                        nr_pages*PAGE_SIZE - sah_Link_Get_Len (ptr) -
                        ((unsigned long)sah_Link_Get_Data (ptr) & ~PAGE_MASK);
                }

#ifdef FLUSH_SPECIFIC_DATA_ONLY
                os_cache_clean_range(buffer_start, buffer_length);
#endif
                ...
                if (link->flags & SAH_OUTPUT_LINK) {
                     ...
                     * 8-word boundary */
                    if ((((uint32_t)buffer_start%32) != 0)
                        && (((uint32_t)buffer_start & 1023) + buffer_length)
                        > 1024) {
#ifdef DO_DBG
                        os_printk("Top of loop: buffer start:"
                                  " %p; buffer length: %d\n",
                                  buffer_start, buffer_length);
#endif

                        /* Shorten current link to 1k boundary */
                        link->len = 1024 - ((uint32_t)buffer_start % 1024);

                        /* Get new link to follow it */
                        link = sah_Alloc_Link();
                        prev_link->len =
                            1024 - ((uint32_t)buffer_start % 1024);
                        prev_link->original_next = link;
                        prev_link->next = (sah_Link *)link->dma_addr;
                        ...
                        link->len = buffer_length;
#ifdef DO_DBG
                        os_printk("buffer start: %p; buffer length: %d\n",
                                  buffer_start, buffer_length);
    ...
    sah_Head_Desc *head_desc)
{
    ...
    while (this_desc != NULL) {
        if (this_desc->ptr1 != NULL) {
            sah_Destroy_Links(this_desc->ptr1);
        }
        if (this_desc->ptr2 != NULL) {
            sah_Destroy_Links(this_desc->ptr2);
        }

        /* Get a bus pointer to the next Descriptor */
        next_desc = this_desc->next;
        ...
        /* Get the Kernel Virtual Address back.  First an approximation.  */
        this_desc = (sah_Desc *)bus_to_virt((uint32_t)next_desc);
        /* Then the official version. */
        this_desc = this_desc->virt_addr;
    }
}
    ...
    sah_Link *link)
{
    ...
    while (this_link != NULL) {
        /* Get the Kernel Virtual Address back */
        this_link = (sah_Link *)bus_to_virt((uint32_t)this_link);
        this_link = this_link->virt_addr;
...
int sah_Block_Add_Page (int big)
{
    ...
    uint32_t dma_offset;
    unsigned block_entries = big ? MEM_BIG_BLOCK_ENTRIES : MEM_BLOCK_ENTRIES;
    ...
    dma_addr = virt_to_bus(page);
#else
    page = os_alloc_coherent(NULL, PAGE_SIZE, &dma_addr, GFP_ATOMIC);
#endif
    if (page != NULL) {
        /*
        ...
        block = page;
        for (block_index = 0; block_index < block_entries; block_index++) {
            if (big) {
                register Mem_Big_Block* blockp = block;
                blockp->dma_addr = (uint32_t)(block - dma_offset);
...
{
    dma_addr_t dma_addr;
    Mem_Big_Block *block = dma_pool_alloc(big_dma_pool, GFP_ATOMIC, &dma_addr);
...
static Mem_Block *sah_Alloc_Block (void)
{
    ...
    dma_addr_t dma_addr;
    Mem_Block *block = dma_pool_alloc(small_dma_pool, GFP_ATOMIC, &dma_addr);
...
*
*/
static void sah_Free_Big_Block(
    Mem_Big_Block *block)
{
    os_lock_context_t lock_flags;
...
*/
static void sah_Append_Big_Block(
    Mem_Big_Block *block)
{
