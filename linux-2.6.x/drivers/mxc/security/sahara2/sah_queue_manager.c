...
fsl_shw_return_t sah_Queue_Manager_Init(void)
{
    ...
    if (main_queue == NULL) {
        ...
        LOG_KDIAG("Trying to initialise the queue manager more than once.");
#endif
    }
...
void sah_Queue_Manager_Close(void)
{
    ...
    if (main_queue && main_queue->count != 0 ) {
        LOG_KDIAG("Trying to close the main queue when it is not empty.");
    }
#endif
...
void sah_process_finished_request(sah_Head_Desc* desc_head, unsigned error)
{
    ...
    if (!error) {
        ...
        desc_head->result = sah_convert_error_status(desc_head->error_status);
#ifdef DIAG_DRV_STATUS
     ...
     */
    desc_head = sah_DePhysicalise_Descriptors(desc_head);
    ...
    if (desc_head->uco_flags & FSL_UCO_BLOCKING_MODE) {
        ...
        os_lock_save_context(desc_queue_lock, lock_flags);
        sah_Queue_Append_Entry(&desc_head->user_info->result_pool, desc_head);
        os_unlock_restore_context(desc_queue_lock, lock_flags);
...
fsl_shw_return_t sah_convert_error_status(uint32_t error_status)
{
    ...
    if (ret == SHA_ERROR_STATUS_CONTINUE) {
        ...
        if (error_source == SAH_ERR_DMA) {
            ...
            DMA_error_size = (error_status >> 9 ) & SAH_DMA_ERR_SIZE_MASK;
            if (DMA_error_size == SAH_DMA_NO_ERR) {
                ...
                ret = sah_DMA_Error_Status_Array[DMA_error_status >> 1];
            }
            ...
            CHA_error_source = (error_status >> 28) & SAH_CHA_ERR_SOURCE_MASK;
        ...
        uint32_t fault_address)
{
    ...
    switch (error_source) {
        ...
        case SAH_ERR_LINK_POINTER:
            sprintf(source_text, "%s", "The data pointer in a link is zero");
        break;
        ...
        case SAH_ERR_OUTPUT_BUFFER:
            sprintf(source_text, "%s", "Output Buffer reported an underflow");
            break;
        ...
        case SAH_ERR_RESERVED_FIELDS:
            sprintf(source_text, "%s", "Reserved pointer field is non-zero");
            break;

        case SAH_ERR_DESCRIPTOR_ADDRESS:
            sprintf(source_text, "%s", "Descriptor address not word aligned");
            break;
        ...
        case SAH_ERR_CHA:
            sprintf(source_text, "%s", "CHA Error");
            {
                ...
                sprintf(cha_log, "  Module %s encountered the error: %s.",
                        cha_module, cha_text);
        ...
        case SAH_ERR_DMA:
            sprintf(source_text, "%s", "DMA Error");
            {
                ...
                if ((dma_direction == NULL) || (dma_size == NULL) ||
                    (dma_text == NULL)) {
                    LOG_KDIAG("No memory allocated for DMA debug messages\n");
                }
                ...
                return_code = sah_DMA_Error_Status_Array[dma_error >> 1];
                dma_error = 1;
    ...
    if (dma_error) {
        ...
        sprintf(address,"Descriptor bus address is 0x%08x", descriptor);
    }
