 ...
 * @return    A return code of type #fsl_shw_return_t.
 */
static inline fsl_shw_return_t process_assoc_from_nist_params(
                                            sah_Link** link1,
                                            uint32_t* data_len,
                                            fsl_shw_uco_t* user_ctx,
                                            fsl_shw_acco_t* auth_ctx,
...
{
    fsl_shw_return_t status;
    uint32_t auth_size_length = COMPUTE_NIST_AUTH_LEN_SIZE(auth_data_length);
    uint32_t auth_pad_length =
        auth_ctx->auth_info.CCM_ctx_info.block_size_bytes - (auth_data_length +
        auth_size_length) % auth_ctx->auth_info.CCM_ctx_info.block_size_bytes;

    if (auth_pad_length == auth_ctx->auth_info.CCM_ctx_info.block_size_bytes) {
        auth_pad_length = 0;
    }
                                            ...
                                            uint32_t auth_data_length)
{
    ...
    temp_buf =
        ...
        ->mem_util->mu_malloc(user_ctx->mem_util->mu_ref,
                              3 *
                              auth_ctx
                              ->auth_info.CCM_ctx_info.block_size_bytes);
    ...
    if (temp_buf == NULL) {
        ...
        cbc_data_length -= auth_ctx->auth_info.CCM_ctx_info.block_size_bytes;

        if (cbc_data_length != 0) {
            while ((status == FSL_RETURN_OK_S) && (cbc_data_length != 0)) {
                uint32_t linklen = (cbc_data_length > CBC_BUF_LEN) ?
                    CBC_BUF_LEN : cbc_data_length;

                status = sah_Append_Link(user_ctx->mem_util, link2,
                                         cbc_buffer,
                                         linklen,
                                   ...
                                   uint8_t* auth_value)
{
    ...
    if (status == FSL_RETURN_OK_S) {
         ...
         * somewhere */
        status = add_in_out_desc(SAH_HDR_MDHA_HASH, payload, payload_length,
                             auth_value, 1, user_ctx->mem_util, &desc_chain);
    }
    ...
    if (status == FSL_RETURN_OK_S) {
        ...
        if (auth_ctx->cipher_ctx_info.mode == FSL_SYM_MODE_CTR) {
            header ^=
                insert_skha_modulus[auth_ctx->cipher_ctx_info.modulus_exp];
        }
                                ...
                                uint8_t* auth_value)
{
    ...
    if (status == FSL_RETURN_OK_S) {
        if (auth_ctx->mode == FSL_ACC_MODE_SSL) {
        ...
        } else if ((auth_ctx->flags & (FSL_ACCO_CTX_INIT | FSL_ACCO_CTX_LOAD |
                                       FSL_ACCO_CTX_SAVE |
            ...
            if (status == FSL_RETURN_OK_S) {
                uint32_t header = SAH_HDR_SKHA_ENC_DEC;
                status = add_in_out_desc(header, garbage_output, 1,
                                         garbage_output, 1,
                                         user_ctx->mem_util, &desc_chain);
            }
            ...
            if ((status == FSL_RETURN_OK_S) && (auth_data_length > 0)) {
                status = add_assoc_preamble(&desc_chain, user_ctx,
                                            auth_ctx, auth_data,
                                            auth_data_length);
            }
    ...
    if (status == FSL_RETURN_OK_S) {
        ...
        if (auth_ctx->mode != FSL_ACC_MODE_CCM) {
            ...
            if (status == FSL_RETURN_OK_S) {
                ...
                status = add_in_out_desc(SAH_HDR_SKHA_ENC_DEC,
                                         auth_value, auth_ctx->mac_length,
                                         auth_ctx->unencrypted_mac,
                                         auth_ctx->mac_length,
                                         user_ctx->mem_util, &desc_chain);
            }
        ...
        if (status != FSL_RETURN_OK_S) {
            ...
            if (calced_auth != NULL) {
                user_ctx->mem_util->
                    mu_free(user_ctx->mem_util->mu_ref, calced_auth);
            }
        } else {
                ...
                if (status == FSL_RETURN_OK_S) {
                    ...
                    for (i = 0; i < auth_ctx->mac_length; i++) {
                        if (calced_auth[i] != auth_ctx->unencrypted_mac[i]) {
                            status = FSL_RETURN_AUTH_FAILED_S;
                            break;
                        }
                    }
                }
