 ...
 * 11/23/2007         Motorola         Changes for handling USB_REQ_SYNCH_FRAME.
 */
/*
...
static int do_standard_device_request (struct usbd_function_instance *function, struct usbd_device_request *request)
{
        ...
        case STATE_DEFAULT:
                ...
                case USB_REQ_SYNCH_FRAME:
                        /* These requests can come in directly in default state for FX2 and Antioch. Bus
                         * drivers for these chips directly handle the set address and the state transition
                         * is from default to configured. So, if a get_status comes immediately after a
                         * set_address it needs to be processed. If a request is coming through set_address
                         * must have succeeded.
                         */ 
                        TRACE_MSG2(USBD, "Device request: %d for this device_state: %d", bRequest, device_state);
                        TRACE_MSG0(USBD, "Set address completed in hardware");
