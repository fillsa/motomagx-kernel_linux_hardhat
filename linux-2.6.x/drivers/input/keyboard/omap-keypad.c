...
static int omap_kp_suspend(struct device *dev, u32 state, u32 level)
{
...
static int omap_kp_resume(struct device *dev, u32 level)
{
	if (level == RESUME_POWER_ON) {
		if (INT_KEYBOARD == omap_pm_get_waker_interrupt())
			pm_set_waker(dev);
