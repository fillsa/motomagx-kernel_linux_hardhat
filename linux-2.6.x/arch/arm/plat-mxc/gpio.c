 ...
 * 29-May-2008  Motorola	Move set_irq_handler into _set_gpio_edge_ctrl._
				 ...
				 struct pt_regs *regs)
{
	...
	if (unlikely(!int_valid)) {
		printk(KERN_INFO "\nGPIO port: %d Spurious interrupt:0x%0x\n\n",
		       port->num, int_valid);
//		BUG();		 // 24-Apr-2008  Motorola        Remove BUG() from mxc_gpio_irq_handler since the spurious gpio inter
