...
extern void mxc27530evb_gpio_init(void) __init;
extern void gpio_mc13783_power_off(void);
...
static struct plat_serial8250_port serial_platform_data[] = {
	{
	 ...
	 .regshift = 0,
	 .iotype = UPIO_MEM,
	 /* REVISIT: Use proper shared IRQ "flags" since
	  * both serial ports share common interrupt line.
	  */
	 ...
	 },
	{
	 ...
	 .irq = INT_GPIO3 + 1,	/* DUMMY value, shared GPIO3 IRQ */
	 .uartclk = 14745600,
	 .regshift = 0,
	 .iotype = UPIO_MEM,
	 /* REVISIT: Use proper shared IRQ "flags" since
...
{
	iomux_config_mux(AP_ED_INT6, OUTPUTCONFIG_FUNC1, INPUTCONFIG_FUNC1);
	set_irq_type(serial_platform_data[0].irq, IRQT_HIGH);

	iomux_config_mux(SP_SIM1_PD, OUTPUTCONFIG_DEFAULT, INPUTCONFIG_DEFAULT);
...
{
	if (__raw_readl(IO_ADDRESS(CRM_COM_BASE_ADDR + 0xC)) & 0x80000) {
 ...
 * initialize __mach_desc_MXC27530EVB data structure.
 */
/* *INDENT-OFF* */
MACHINE_START(MXC27530EVB, "Freescale MXC275-30 EVB")
