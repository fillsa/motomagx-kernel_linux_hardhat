...
enum iomux_reg_addr {
	...
	IOMUXSW_AP_PAD_CTL = IO_ADDRESS(IOMUX_AP_BASE_ADDR) + 0x200,	/*!< Pad control for AP */
	IOMUXSW_SP_PAD_CTL = IO_ADDRESS(IOMUX_COM_BASE_ADDR) + 0x200,	/*!< Pad control for SP */
...
__u16 iomux_get_pad(enum iopad_group grp)
{
	...
	if (grp <= AP_MAX_PAD_GROUP) {
		base = (volatile __u16 *)(IOMUXSW_AP_PAD_CTL);
	} else {
		base = (volatile __u16 *)(IOMUXSW_SP_PAD_CTL);
		grp -= (AP_MAX_PAD_GROUP + 1);
	}

	return (__u16) (base[grp]);
}
