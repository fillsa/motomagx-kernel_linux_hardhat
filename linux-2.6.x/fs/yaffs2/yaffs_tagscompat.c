 ...
 * 08-28-2006   Motorola  Add additional trace statements for debugging
 */
				   ...
				   yaffs_Tags * tagsPtr)
{
	...
	if (result > 0) {
        ...
        {
            __u32 *pBytes;
            pBytes = (__u32 *)sparePtr;
 
            if ((pBytes[0] != 0xffffffff) ||
                (pBytes[1] != 0xffffffff) ||
                (pBytes[2] != 0xffffffff) ||
                (pBytes[3] != 0xffffffff))
            {
                printk("%s: (After Read)\n", __FUNCTION__);
                printk("  Spare: tagByte0=0x%x, tagByte1=0x%x, tagByte2=0x%x, tagByte3=0x%x\n",
                          sparePtr->tagByte0, sparePtr->tagByte1,
                          sparePtr->tagByte2,sparePtr->tagByte3);
                printk("  Spare:  tagByte4=0x%x, tagByte5=0x%x, tagByte6=0x%x, tagByte7=0x%x\n",
                          sparePtr->tagByte4, sparePtr->tagByte5,
                          sparePtr->tagByte6, sparePtr->tagByte7);
                printk("  Spare:  pageStatus=0x%x\n",sparePtr->pageStatus);
                printk("  Tags: chunkId=%d, SN=%d, byteCount=%d, OId=%d\n",
                      tagsPtr->chunkId, tagsPtr->serialNumber, tagsPtr->byteCount,
                      tagsPtr->objectId);
            }
