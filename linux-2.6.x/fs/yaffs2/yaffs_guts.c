 ...
 * 07-18-2006   Motorola  Added shredding support to YAFFS2
 *              Added BBT support to YAFFS2
 * 12-15-2006   Motorola  Do not shred during mount
 * 01-23-2007   Do not shred Object Header pages
 */
...
/* FIXME: Yaffs does not support zero page */
	...
	if (markNAND &&
...
#ifdef CONFIG_MOT_FEAT_YAFFS_SHREDDER
		/* This is the YAFFS1 (small page) shredding. */
		if ((dev->sMountOptions & YAFFS_MOUNT_SHRED) &&
		   ...
		   if (newTags.chunkId != 0) {
		        ...
		        printk("Going to SHRED: chunkInNand=%d, chunkId=%d\n",
		              (chunkId - dev->chunkOffset), newTags.chunkId);
#endif

		        kaddr = kmap_atomic (ZERO_PAGE(0), KM_USER0);
		        yaffs_WriteChunkWithTagsToNAND(dev, chunkId, (__u8 *) kaddr, &tags);
	   ...
	   yaffs_HandleUpdateChunk(dev, chunkId, &tags);
...
#if defined(CONFIG_MOT_FEAT_YAFFS_SHREDDER) && defined(CONFIG_MOT_WFN439)
		...
		if (newTags.chunkId != 0) {
		    ...
		    /* If the page is a header object page, we don't shred.
		     * With the Large page case, we don't shred the OOB
		     * area of the data page.  We only shred the main area
		     * of the data page.
		     */
